...
func init() {
	...
	AddCustomRule("alpha_dash", func(field string, vlaue string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 只能由字母、数字、短划线(-)和下划线(_)组成", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isAlphaDash(str) {
	...
	AddCustomRule("alpha_space", func(field string, vlaue string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 只能包含字母、数字、破折号和空格", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isAlphaSpace(str) {
	...
	AddCustomRule("alpha_num", func(field string, vlaue string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 只能由字母和数字组成", field)
		if message != "" {
			err = errors.New(message)
		}
	...
	AddCustomRule("bool", func(field string, vlaue string, message string, value interface{}) error {
		err := fmt.Errorf("%s 只能包含布尔值、字符串或整数 0、1", field)
		if message != "" {
			err = errors.New(message)
		}
		switch t := value.(type) {
	...
	AddCustomRule("between", func(field string, rule string, message string, value interface{}) error {
		...
		err = fmt.Errorf("%s 必须介于 %d - %d 之间", field, min, max)
		if message != "" {
			err = errors.New(message)
		}
		rv := reflect.ValueOf(value)
		...
		case reflect.Float32:
			...
			if !(in >= minFloat && in <= maxFloat) {
				return fmt.Errorf("%s 必须介于 %f - %f 之间", field, minFloat, maxFloat)
			}
		case reflect.Float64:
			in := value.(float64)
			if !(in >= minFloat && in <= maxFloat) {
	...
	AddCustomRule("credit_card", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 字段必须是有效的信用卡号码", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isCreditCard(str) {
	...
	AddCustomRule("coordinate", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 字段必须是有效的坐标", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isCoordinate(str) {
	...
	AddCustomRule("css_color", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 字段必须是有效的 CSS 颜色代码", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isCSSColor(str) {
	...
	AddCustomRule("digits", func(field string, rule string, message string, value interface{}) error {
		...
		err = fmt.Errorf("%s 必须是%d位的数字", field, l)
		if l == 1 {
			err = fmt.Errorf("%s 字段必须是1位数字", field)
		}
		if message != "" {
			err = errors.New(message)
		}
	...
	AddCustomRule("digits_between", func(field string, rule string, message string, value interface{}) error {
		...
		err = fmt.Errorf("%s 必须是介于%d和%d位的数字", field, min, max)
		if message != "" {
			err = errors.New(message)
		}
		str := toString(value)
	...
	AddCustomRule("date", func(field string, rule string, message string, value interface{}) error {
		...
		case "date:dd-mm-yyyy":
			if !isDateDDMMYY(str) {
				...
				return fmt.Errorf("%s 不是一个有效的日期", field)
			}
		default:
			if !isDate(str) {
				if message != "" {
	...
	AddCustomRule("email", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 不是一个合法的邮箱", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isEmail(str) {
	...
	AddCustomRule("float", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是一个浮点数", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isFloat(str) {
	...
	AddCustomRule("ip", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 IP 地址", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isIP(str) {
	...
	AddCustomRule("ip_v4", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 IPv4 地址", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isIPV4(str) {
	...
	AddCustomRule("ip_v6", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 IPv6 地址", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isIPV6(str) {
	...
	AddCustomRule("json", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是正确的 JSON 格式", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isJSON(str) {
	...
	AddCustomRule("lat", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须包含有效的纬度", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isLatitude(str) {
	...
	AddCustomRule("lon", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须包含有效的经度", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isLongitude(str) {
	...
	AddCustomRule("len", func(field string, rule string, message string, value interface{}) error {
		...
		err = fmt.Errorf("%s 长度必须为%d", field, l)
		if message != "" {
			err = errors.New(message)
		}
		rv := reflect.ValueOf(value)
	...
	AddCustomRule("min", func(field string, rule string, message string, value interface{}) error {
		...
		errMsg := fmt.Errorf("%s 必须大于等于%d", field, lenInt)
		if message != "" {
			errMsg = errors.New(message)
		}
		errMsgFloat := fmt.Errorf("%s 值不能小于 %f", field, lenFloat)
		if message != "" {
		...
		case reflect.String:
			...
			if inLen < lenInt {
				...
				return fmt.Errorf("%s 至少为%d字符", field, lenInt)
			}
		case reflect.Array, reflect.Map, reflect.Slice:
			...
			if inLen < lenInt {
				...
				return fmt.Errorf("%s 大小必须至少为%d", field, lenInt)
			}
		case reflect.Int:
			in := value.(int)
			if in < lenInt {
	...
	AddCustomRule("max", func(field string, rule string, message string, value interface{}) error {
		...
		errMsg := fmt.Errorf("%s 值不能大于%d", field, lenInt)
		if message != "" {
			errMsg = errors.New(message)
		}
		errMsgFloat := fmt.Errorf("%s 值不能大于 %f", field, lenFloat)
		if message != "" {
			errMsgFloat = errors.New(message)
		}
		rv := reflect.ValueOf(value)
		...
		case reflect.String:
			...
			if inLen > lenInt {
				...
				return fmt.Errorf("%s 不能大于%d字符", field, lenInt)
			}
		case reflect.Array, reflect.Map, reflect.Slice:
			inLen := rv.Len()
			if inLen > lenInt {
				...
				return fmt.Errorf("%s 长度不能大于%d", field, lenInt)
			}
		case reflect.Int:
			in := value.(int)
			if in > lenInt {
	...
	AddCustomRule("mac_address", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 MAC 地址", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isMacAddress(str) {
	...
	AddCustomRule("numeric_between", func(field string, rule string, message string, value interface{}) error {
		...
		switch {
		case rng[0] == "":
			errMsg = fmt.Errorf("%s 值不能大于 %d", field, max)
		case rng[1] == "":
			errMsg = fmt.Errorf("%s 值不能小于 %d", field, min)
		default:
			errMsg = fmt.Errorf("%s 必须是介于 %d 和 %d 之间的数字", field, min, max)
		}
		...
		switch {
		case rng[0] == "":
			errMsg = fmt.Errorf("%s 字段的值不能大于 %f", field, maxFloat)
		case rng[1] == "":
			errMsg = fmt.Errorf("%s 字段的值不能小于 %f", field, minFloat)
		default:
			errMsg = fmt.Errorf("%s 字段必须是介于 %f 和 %f 之间的数字", field, minFloat, maxFloat)
		}
	...
	AddCustomRule("uuid", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 UUID", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isUUID(str) {
	...
	AddCustomRule("uuid_v3", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 UUID V3", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isUUID3(str) {
	...
	AddCustomRule("uuid_v4", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 UUID V4", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isUUID4(str) {
	...
	AddCustomRule("uuid_v5", func(field string, rule string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("%s 必须是有效的 UUID V5", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isUUID5(str) {
	...
	AddCustomRule("in", func(field string, rule string, message string, value interface{}) error {
		...
		str := toString(value)
		err := fmt.Errorf("%s 选项必须在 %v 里", field, strings.Join(rng, ", "))
		if message != "" {
			err = errors.New(message)
		}
		if !isIn(rng, str) {
	...
	AddCustomRule("not_in", func(field string, rule string, message string, value interface{}) error {
		...
		str := toString(value)
		err := fmt.Errorf("%s 选项不能在 %v 里", field, strings.Join(rng, ", "))
		if message != "" {
			err = errors.New(message)
		}
		if isIn(rng, str) {
